...
//  public PVector GetCameraRotationYawPitchRoll(int targetID) // yaw, pitch roll angles of camera in target coordinate system 
...
public class Tracker {
  ...
  private String path = "marker/img/500x500/";
  ...
  private void Init_cparam(int width, int height) 
  {
    ...
    // parameters calibrated through Matlab 2013b (equal to OpenCV)
    ...
    intrinsic_matrix[0] = 1115.56510417658 * s[0]; // focal length x
    intrinsic_matrix[1] = 2.21283125572226 * s[0]; // skew
    intrinsic_matrix[2] = 630.686406791672 * s[0]; // principal point x
    intrinsic_matrix[3] = 0.0;
    intrinsic_matrix[4] = 1118.46475973006 * s[1]; // focal length y
    intrinsic_matrix[5] = 345.564443137407 * s[1]; // principal point y
    ...
    distortion_coeffs[0] =-0.554709703900109; // radial 1
    distortion_coeffs[1] = 0.328612123243337; // radial 2
    distortion_coeffs[2] = 0.00523432043894078; // tangential 1
    distortion_coeffs[3] = 3.15345378957248e-05; // tangential 2
    return;
  }
  ...
  private void LogFrame() 
  {
    ...
    for (int i=0; i<numTargets; i++) {
      ...
      //log.println("Time [ms], Frame number, Target ID, Target marker ID, Distance origin, Distance plane, Confidence, Life, Lost count, [events]");
      String logFrame =
        millis() + ", " + 
        frameNum + ", " + 
        i + ", " + 
        GetTargetCurrentMarkerID(i) + ", " + 
        GetTargetDistance(i) + ", " + 
        GetTargetPlaneDistance(i) + ", " + 
        GetTargetConfidence(i) + ", " + 
        GetTargetLife(i) + ", " + 
        GetTargetLostCount(i);
      for (int j=0; j<logEvents.length; j++) {
        logFrame += (", " + -1);
      }
      log.println(logFrame);
      log.flush();
    }
    return;
  }
  ...
  public PVector GetCameraRotationYawPitchRoll(int targetID)
  {    
    PMatrix3D m = t[targetID].C2T;
    
    float yaw = atan2(m.m10, m.m00);
    float pitch = atan2(-m.m20, sqrt(m.m21*m.m21 + m.m22*m.m22));
    float roll = atan2(m.m21, m.m22);
    
    return new PVector(yaw, pitch, roll);
  } 
